...
##  This script is specifically tailored for running by Hufflagle Lab members but
...
if [ -z $SILVAPATH ]; then # set Silva database path
	SILVAPATH='/nfs/turbo/umms-ghuff/Databases/MothurDB/SilvaV132/silva.nr_v132.regionV4.align'
fi
if [ -z $RDPREFPATH ]; then # set RDP database fasta path
	RDPREFPATH='/nfs/turbo/umms-ghuff/Databases/MothurDB/RDP16/trainset16_022016.rdp.fasta'
fi
if [ -z $RDPTAXPATH ]; then # set RDP database taxonomy path
	RDPTAXPATH='/nfs/turbo/umms-ghuff/Databases/MothurDB/RDP16/trainset16_022016.rdp.tax'
fi
